...
$modo_edicao = true; //ativa a edição no form
break;
}
}
...
if ($pessoa ['id'] == $id_para_atualizar){
    $_SESSION['pessoas'][$indice]['nome'];    
    $_SESSION['pessoas'][$indice]['email'];    
    $_SESSION['pessoas'][$indice]['celular'];    
break;
}
}
}
...
form{margin-bottom: 20px; padding: 20px; border: 1px solid #ccc; border-radius: 3px;}
form div{ margin-bottom: 10px;}
label{display: block; margin-bottom: 5px; }
input[type="text"], input[type="email"]{width: calc(100% - 16px);padding: 8px; border:}
button{padding: 10px 15px; background-color: #28a745; color: white; border: none; border:}
button.update{background-color: #007bff;}
table{ width: 100%; border-colapse: collapse;}
...
<imput type="hidden name="id" value="<?php echo htmlspecialchars($nome_edicao); ?>" requ  